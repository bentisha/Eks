...
resource "aws_eip" "nat_eip" {
  ...
  domain = "vpc"
  tags = {
    Name = "hamza-nat-eip-${count.index}-${var.env}"
  }
...
resource "aws_nat_gateway" "nat" {
  ...
  tags = {
    Name = "hamza-nat-${count.index}-${var.env}"
  }
...
resource "aws_route_table" "public" {
  ...
  tags = {
    Name = "hamza-public-route-table-${var.env}"
  }
...
resource "aws_security_group_rule" "public_sg_rules" {
  ...
  from_port         = contains(each.value.port_range, "-") ? tonumber(split("-", each.value.port_range)[0]) : tonumber(each.value.port_range)
  to_port           = contains(each.value.port_range, "-") ? tonumber(split("-", each.value.port_range)[1]) : tonumber(each.value.port_range)
...
resource "aws_security_group_rule" "private_sg_rules" {
  ...
  cidr_blocks = (each.value.dst_cidr != "" && each.value.dst_sg == "") ? [each.value.dst_cidr] : null
  source_security_group_id = (each.value.dst_sg != "" && each.value.dst_cidr == "") ? (
    each.value.dst_sg == "public_sg" ? aws_security_group.public_sg.id : null
  ) : null
}